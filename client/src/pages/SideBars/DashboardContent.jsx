...
//         dispatch({ type: 'auth/updateWallet', payload: { wallet: response.data.wallet } });
//       }
//     } catch (error) {
...
//     ...(dashboardData?.user?.wallet?.transactions || []) // Merge Paystack and Paylony transactions
//   ];
...
//   const dispatch = useDispatch();
//   const navigate = useNavigate();
//   const { loading, dashboardData, error, walletBalance } = useSelector((state) => state.auth);
//   const token = localStorage.getItem('token');
//   const [fundingAmount, setFundingAmount] = useState('');
//   const [fundingUrl, setFundingUrl] = useState(null);
//   const [fundingReference, setFundingReference] = useState(null);
...
//   const virtualBalance = dashboardData?.user?.paylonyVirtualAccountDetails?.balance || 0;
//   const totalBalance = walletBalance + virtualBalance; // Use walletBalance from state for accuracy
//   const allTransactions = [
//     ...(dashboardData?.user?.wallet?.transactions || []),
...
function DashboardContent() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { loading, dashboardData, error, walletBalance } = useSelector((state) => state.auth); // walletBalance is assumed to be updated by fetchWalletBalance
  const token = localStorage.getItem('token');
  const [fundingAmount, setFundingAmount] = useState('');
  const [fundingUrl, setFundingUrl] = useState(null);
  const [fundingReference, setFundingReference] = useState(null);

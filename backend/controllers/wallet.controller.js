 ...
 * /api/bills/pay:
 *   post:
 *     summary: Pay a bill using wallet balance
 *     tags: [Wallet]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *           schema:
 *             type: object
 *             required: [billType, provider, phone, amount]
 *             properties:
 *               billType:
 *                 type: string
 *                 example: "electricity"
 ...
 *                   type: number
 *       400:
 *         description: Insufficient balance or user not found
 *         content:
 *           application/json:
 *             schema:
 ...
 *   post:
 *     summary: Paystack webhook — credits wallet on charge.success or transfer.success
 *     description: >
 *       Not bearer-authenticated. Matches the paying customer's email (for charge.success)
 *       or destination account number (for transfer.success) and credits the wallet.
 *     tags: [Wallet]
 *     security: []
 *     requestBody:
 *       required: true
...
/**
 * @swagger
 * /api/wallet/fund-wallet:
 *   post:
 *     summary: Initialize a Paystack payment to fund the wallet
 *     tags: [Wallet]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *                 url:
 *                   type: string
 *                   description: Paystack checkout URL to redirect the user to
 *                 reference:
 *                   type: string
 *       400:
...
/**
 * @swagger
 * /api/wallet/fetch-paylony-accounts:
 *   get:
 *     summary: Fetch the user's linked Paylony virtual account details
 *     tags: [Wallet]
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 *         description: Wallet info for the linked Paylony account
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                         type: object
 *       400:
 *         description: No Paylony account linked to this user
 *       500:
 *         description: Failed to fetch Paylony accounts
 */
...
/**
 * @swagger
 * /api/wallet/webhook/paylony:
 *   post:
 *     summary: Paylony inward-transfer webhook — credits wallet on successful collection
 *     description: >
 *       Not bearer-authenticated. Matches the user by their Paylony virtual account
 *       number and credits (amount - fee) to their wallet balance.
 *     tags: [Wallet]
 *     security: []
 *     requestBody:
 *       required: true
 ...
 *     responses:
 *       200:
 *         description: Webhook processed (always returns 200 even if the account wasn't matched)
 *       500:
...
/**
 * @swagger
 * /api/wallet/manual-update:
 *   post:
 *     summary: Manually credit the wallet for a specific hardcoded transaction reference
 *     description: >
 *       ⚠️ This endpoint only accepts a single hardcoded reference ("C2509060510") and
 *       is intended as a one-off manual reconciliation tool, not a general-purpose credit endpoint.
 *     tags: [Wallet]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *     responses:
 *       200:
 *         description: Wallet manually updated
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   type: object
 *       400:
 *         description: Reference does not match the accepted hardcoded value
 *       404:
 *         description: User not found
 *       500:
...
/**
 * @swagger
 * /api/wallet/webhook/paylony2:
 *   post:
 *     summary: Paylony webhook (v2) — deduplicated inward-transfer credit with balance sync
 *     description: >
 *       Same trigger as handlePaylonyWebhook but adds duplicate-reference protection
 *       and attempts to fetch the authoritative balance from Paylony before falling
 *       back to a locally-calculated balance.
 *     tags: [Wallet]
 *     security: []
 *     requestBody:
 *       required: true
...
/**
 * @swagger
 * /api/wallet/directdebit/create-mandate:
 *   post:
 *     summary: Create a Paylony direct debit mandate for recurring wallet debits
 *     tags: [Wallet]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *           schema:
 *             type: object
 *             required: [payerBank, payerAccountNumber, payerBvn, payerPhone, amount, startDate, endDate, frequency, payerSignature, witnessSignature]
 *             properties:
 *               reference:
 *                 type: string
 ...
 *         description: User not found
 *       500:
 *         description: Mandate creation failed
 */
...
/**
 * @swagger
 * /api/wallet/directdebit/debit-mandate:
 *   post:
 *     summary: Trigger a debit against an existing direct debit mandate
 *     tags: [Wallet]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *                   type: object
 *       400:
 *         description: No mandate found for this user
 *       500:
 *         description: Debit mandate failed
 */
 ...
 * /api/wallet/callback:
 *   get:
 *     summary: Paylony payment callback — reconciles wallet balance after redirect
 *     tags: [Wallet]
 *     security: []
 *     parameters:
 *       - in: query
 ...
 *     responses:
 *       200:
 *         description: Wallet reconciled from callback
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   type: object
 *       404:
 *         description: User not found for this reference
 *       500:
 *         description: Failed to fetch balance / process callback
 */
...
/**
 * @swagger
 * /api/wallet/fetch-and-update-balance/{reference}/{provider}:
 *   get:
 *     summary: Fetch and reconcile wallet balance from Paylony or Paystack
 *     description: >
 *       Uses a MongoDB transaction. Determines provider automatically from the
 *       user's linked accounts if not supplied via query. Falls back to the
 *       currently-stored wallet balance if the upstream provider call fails.
 *     tags: [Wallet]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         schema:
 *           type: string
 *           enum: [paylony, paystack]
 *     responses:
 *       200:
 *         description: Balance fetched and reconciled
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *   get:
 *     summary: Reconcile wallet balance against Paystack dedicated virtual account transactions
 *     description: >
 *       Sums all successful dedicated_nuban transactions for the user's Paystack
 *       customer ID and account number, then updates the wallet balance and logs
 *       a credit/debit/balance_check transaction accordingly. Uses a MongoDB transaction.
 *     tags: [Wallet]
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 *         description: Balance retrieved and reconciled
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                     balance:
 *                       type: number
 *                       description: Balance in NGN (converted from kobo)
 *                     transaction_count:
 *                       type: integer
 *                     last_updated:
 *                       type: string
 *                       format: date-time
 *       500:
 *         description: Failed to fetch balance (unauthorized, no virtual account, or Paystack error)
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/Error'

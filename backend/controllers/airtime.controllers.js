...
/**
 * @swagger
 * /api/airtime/buy-airtime:
 *   post:
 *     summary: Purchase airtime using wallet balance
 *     description: >
 *       Deducts the amount from the user's wallet, validates the number/provider
 *       with the MCD reseller API, then places the airtime order. On MCD failure,
 *       the wallet deduction is rolled back and the transaction is marked failed.
 *     tags: [Airtime]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *               network:
 *                 type: string
 *                 description: Telecom provider code (e.g. "mtn", "glo", "airtel", "9mobile")
 *                 example: "mtn"
 *               amount:
 *                 type: number
 *                 example: 500
 ...
 *                   type: string
 *       400:
 *         description: Missing/invalid fields, insufficient balance, or duplicate reference
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/Error'
 ...
 *         description: User not found
 *       500:
 *         description: Airtime purchase failed (validation or MCD API error)
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/Error'
...
/**
 * @swagger
 * /api/airtime/buy-data-pin:
 *   post:
 *     summary: Purchase a data pin using wallet balance
 *     description: >
 *       Same flow as buyAirtime — validates via MCD, deducts wallet balance,
 *       calls MCD's /data endpoint, and rolls back on failure.
 *     tags: [Data]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *           schema:
 *             type: object
 *             required: [provider, amount, number, country]
 *             properties:
 *               provider:
 *                 type: string
 *                 example: "mtn"
 ...
 *     responses:
 *       200:
 *         description: Data pin purchase successful
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   type: string
 *       400:
 *         description: Missing/invalid fields, invalid number/provider, or insufficient balance
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/Error'
